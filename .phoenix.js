...
const moveToNextDisplay = (window) => {
  ...
  // const condition = (screen) => screen.identifier() !== screenCurrent.identifier();
  // const nextScreen = screens.find(condition)
  //
  const nextScreen = screens.find((_, idx) => {
    if (idx === 0) { return screens[1] }
    if (idx === 1) { return screens[2] }
    if (idx === 2) { return screens[0] }
  });
...
// 3 monitors assign to number 
function moveWindowToScreenIndex(targetIndex) {
  const screens = Screen.all();
  const targetScreen = screens[targetIndex];
  const window = Window.focused();

  if (!targetScreen || !window) return;

  const frame = targetScreen.flippedVisibleFrame();
...
// Key bindings for triple monitor setup
Key.on('1', MOD, () => moveWindowToScreenIndex(0)); // Primary
Key.on('2', MOD, () => moveWindowToScreenIndex(1)); // Secondary
Key.on('3', MOD, () => moveWindowToScreenIndex(2)); // Tertiary
